...
{
private:
	const fg::Mesh2 *mesh;	// указатель ли?
...
void display(void)
{
	...
	const fg::Mesh2 *mesh = globalMeshDrawer.getMesh();
	...
	{
		glVertex3d((i + xChange) * scale, (yMin - 6 + yChange) * scale, 0.0);
		glVertex3d((i + xChange) * scale, (yMax + 6 + yChange) * scale, 0.0);
	}

	for (int i = yMin - 6; i < yMax + 6; i++)
	{
		glVertex3d((xMin - 6 + xChange) * scale, (i + yChange) * scale, 0.0);
		glVertex3d((xMax + 6 + xChange) * scale, (i + yChange) * scale, 0.0);
	}
	...
	glColor3d(0.0, 0.0, 0.0);
	glVertex3d((xMin - 6 + xChange) * scale, (0.0 + yChange) * scale, 0.0);
	glVertex3d((xMax + 6 + xChange) * scale, (0.0 + yChange) * scale, 0.0);

	glVertex3d((0.0 + xChange) * scale, (yMin - 6 + yChange) * scale, 0.0);
	glVertex3d((0.0 + xChange) * scale, (yMax + 6 + yChange) * scale, 0.0);
